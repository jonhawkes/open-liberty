...
public class LeaseTimeoutManager {
    ...
    private static LeaseRenewer _leaseRenewer;
    private static LeaseChecker _leaseChecker;
    ...
    public static void setTimeouts(SharedServerLeaseLog leaseLog, String recoveryIdentity, String recoveryGroup, RecoveryAgent recoveryAgent, RecoveryDirector recoveryDirector,
        ...
        _leaseRenewer = new LeaseRenewer(leaseLength, leaseLog);
        _leaseChecker = new LeaseChecker(leaseCheckInterval, recoveryAgent, recoveryDirector);

        if (tc.isEntryEnabled())
            Tr.exit(tc, "setTimeouts");
    }

    public static void stopTimeouts() {
        if (tc.isEntryEnabled())
            Tr.entry(tc, "stopTimeouts");

        if (_leaseRenewer != null) {
            _leaseRenewer.cancelAlarm();
        }

        if (_leaseChecker != null) {
            _leaseChecker.cancelAlarm();
        }

        if (tc.isEntryEnabled())
            Tr.exit(tc, "stopTimeouts");
    }
